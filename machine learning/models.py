...
class PerceptronModel(object):
    ...
    def get_prediction(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        if not nn.as_scalar(self.run(x)) < 0:
            return 1
        return -1
...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        l = nn.ReLU(nn.AddBias(nn.Linear(x, self.w1), self.b1))
        y = nn.AddBias(nn.Linear(l, self.w2), self.b2)
        return y
        ...
        while err >= 0.001:
            for x, y in dataset.iterate_once(200):
                ...
                for i in range(len(self.params)):
                    self.params[i].update(grads[i], -0.01)
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        # Initialize your model parameters here
        "*** YOUR CODE HERE ***"
        self.h0 = nn.Parameter(1, 50)
        self.wxh = nn.Parameter(47, 50)
        self.whh = nn.Parameter(50, 50)
        self.bh = nn.Parameter(1, 50)

        self.why = nn.Parameter(50, 5)
        self.by = nn.Parameter(1, 5)
    ...
    def run(self, xs):
        ...
        """
        "*** YOUR CODE HERE ***"
        h = nn.ReLU(nn.AddBias(nn.Linear(xs[0], self.wxh), self.bh))
        for x in xs:
            h = nn.ReLU(nn.AddBias(nn.Add(nn.Linear(x, self.wxh), nn.Linear(h, self.whh)), self.bh))
        y = nn.AddBias(nn.Linear(h, self.why), self.by)
