...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostbeliefs in livingGhostPositionDistributions:
            mostlikelypos = ghostbeliefs.argMax()
            tmp = self.distancer.getDistance(pacmanPosition, mostlikelypos)
            if tmp < dclosestghost:
                dclosestghost = tmp
                gpos = mostlikelypos
        ...
        action = None
        for a in legal:
            npos = Actions.getSuccessor(pacmanPosition, a)
            temp = self.distancer.getDistance(npos, gpos)
            if temp < d:
                d = temp
                action = a
        return action
